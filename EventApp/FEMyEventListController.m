...
@interface FEMyEventListController ()
...
@implementation FEMyEventListController
...
- (void)viewDidLoad
{
    ...
    self.title = NSLocalizedString(@"我的活动", @"我的活动");
...
{
    static NSString *CellIdentifier = @"MyEventTableViewCell";
...
{
    FEEvent *event = [self.eventData objectAtIndex:indexPath.row];
	FEMyEventDetailController *detailView = [[[FEMyEventDetailController alloc] init] autorelease];
...
- (void)loadEvent
{
    ...
    NSString *loadURL = [NSString stringWithFormat:@"%@%@", API_BASE, API_EVENT_OWNER];
    
    self.dataRequest = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:loadURL]];
    self.dataRequest.requestMethod = @"POST";
    [self.dataRequest setPostValue:[NSNumber numberWithInt:user_id] forKey:@"owner_id"];
